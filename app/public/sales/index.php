...
$users = DB::all("SELECT id, full_name FROM users WHERE role IN ('admin','supervisor','seller') ORDER BY full_name");
?>
...
<style>
  ...
  .status-badge { padding: 3px 8px; border-radius: 99px; color: #fff; font-size: 0.8rem; }
...
async function loadSales() {
  ...
  try {
    ...
    if (j.data.length > 0) {
        tfoot.innerHTML = `<tr class="total-row"><td colspan="5">Total de ventas listadas:</td><td>${mfmt(totalAmount)}</td><td colspan="2"></td></tr>`;
    } else {
...
function cancelSale(id, status) {
  if (status === 'COMPLETED') {
    Swal.fire('Acción no permitida', 'No se puede anular una venta con factura fiscal (CAE) emitida. Debes generar una Nota de Crédito.', 'warning');
    return;
  }
  
  Swal.fire({
    title: '¿Estás seguro?',
    text: `Se anulará la venta #${id}. Esta acción no se puede revertir.`,
    icon: 'warning',
    ...
    if (result.isConfirmed) {
      // Próximamente: Llamada a la API de anulación
      Swal.fire('Próximamente', 'La función de anular estará disponible en la siguiente fase.', 'info');
    }
  });
}
